  ...
  return (
    <div className="min-h-screen bg-canvas">
      ...
      <div className="mx-auto max-w-5xl px-4 py-10">
        <h1 className="mb-6 text-2xl font-bold text-fg">Избранное</h1>

        {items === null && <p className="text-fg-subtle">Загрузка…</p>}

        {items !== null && items.length === 0 && (
          <div className="text-fg-subtle">
            <p>Здесь пока пусто.</p>
            <Link to="/search" className="text-link hover:underline">Перейти к поиску</Link>
          </div>
        )}
        ...
        {items !== null && items.length > 0 && (
          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {items.map((item) => (
              <article key={item.id}
                className={`flex flex-col overflow-hidden rounded-2xl border bg-surface shadow-sm ${item.available ? "border-edge" : "border-slate-200 opacity-75"}`}>
                ...
                <div className="flex flex-1 flex-col gap-3 p-4">
                  <h3 className="text-base font-semibold leading-snug text-fg">{item.title}</h3>
                  <div className="mt-auto flex items-center justify-between gap-3 pt-2">
                    <span className="text-lg font-bold text-link">{formatPrice(item.price, item.currency)}</span>
                    {item.available ? (
                      ...
                      <span className="inline-flex cursor-not-allowed items-center rounded-full bg-slate-200 px-4 py-2 text-sm font-medium text-fg-faint">
                        Купить
                      </span>
                    )}
                  </div>
                  <button onClick={() => remove(item.id)}
                    className="text-sm text-fg-faint hover:text-sale">
