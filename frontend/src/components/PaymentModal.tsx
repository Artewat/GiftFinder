...
export default function PaymentModal({ amountLabel, onClose, onConfirm }: Props) {
  ...
  function inputCls(f: Field, extra = ""): string {
    ...
    return (
      ...
      (bad
        ...
        : "border-violet-200 dark:border-slate-700 focus:border-violet-400 focus:ring-violet-200")
    );
  }
  ...
  return (
    ...
    >
      ...
      />
      <div className="relative w-full max-w-md rounded-t-2xl bg-white dark:bg-slate-800 p-6 shadow-xl sm:rounded-2xl">
        <div className="flex items-start justify-between">
          <div>
            <h2 id="pay-title" className="text-xl font-bold text-violet-950 dark:text-slate-100">
              Оформление Premium
            </h2>
          ...
          <button
            ...
            disabled={processing}
            aria-label="Закрыть"
            className="-mr-1 -mt-1 inline-flex h-9 w-9 items-center justify-center rounded-lg text-slate-400 dark:text-slate-500 transition-colors hover:bg-violet-50 dark:hover:bg-slate-800 hover:text-violet-700 dark:hover:text-violet-300 disabled:opacity-40"
          >
        ...
        <div className="mt-3 rounded-lg bg-amber-50 dark:bg-amber-950/30 px-3 py-2 text-xs text-amber-700 dark:text-amber-300">
        ...
        <form onSubmit={pay} noValidate className="mt-4 space-y-3">
          <label className="block">
            <span className="mb-1 block text-sm font-medium text-violet-950 dark:text-slate-100">Номер карты</span>
            <div className="relative">
              ...
              />
              {brand && (
                <span className="pointer-events-none absolute right-3 top-[1.35rem] -translate-y-1/2 text-xs font-semibold text-violet-500 dark:text-violet-300">
                  {brand}
                </span>
              )}
            </div>
            {show("number") && (
              <p className="mt-1 text-xs text-rose-600 dark:text-rose-400">{show("number")}</p>
            )}
          </label>

          <div className="flex gap-3">
            <label className="block flex-1">
              <span className="mb-1 block text-sm font-medium text-violet-950 dark:text-slate-100">Срок</span>
              <input
                inputMode="numeric"
                autoComplete="cc-exp"
                placeholder="ММ/ГГ"
              ...
              />
              {show("expiry") && (
                <p className="mt-1 text-xs text-rose-600 dark:text-rose-400">{show("expiry")}</p>
              )}
            </label>
            <label className="block w-24">
              <span className="mb-1 block text-sm font-medium text-violet-950 dark:text-slate-100">CVC</span>
              <input
                inputMode="numeric"
                autoComplete="cc-csc"
                placeholder="123"
          ...
          {show("cvc") && (
            <p className="-mt-1 text-xs text-rose-600 dark:text-rose-400">{show("cvc")}</p>
          )}

          <label className="block">
            <span className="mb-1 block text-sm font-medium text-violet-950 dark:text-slate-100">Имя на карте</span>
            <input
            ...
            />
            {show("name") && (
              <p className="mt-1 text-xs text-rose-600 dark:text-rose-400">{show("name")}</p>
            )}
          </label>

          {error && (
            <p className="rounded-lg bg-rose-50 dark:bg-rose-950/30 px-3 py-2 text-sm text-rose-700 dark:text-rose-300">{error}</p>
          )}
          ...
          <p className="text-center text-xs text-slate-400 dark:text-slate-500">🔒 Демонстрационная оплата, реальное списание не производится</p>
        </form>
      </div>
    </div>
  );
